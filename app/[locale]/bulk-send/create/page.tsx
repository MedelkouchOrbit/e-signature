...
import {
  templatesApiService,
  type OpenSignTeamMember,
} from "@/app/lib/templates-api-service";
...
export default function CreateBulkSendPage() {
  ...
  const loadTemplates = useCallback(async () => {
    try {
      ...
      if (!response || !response.results) {
        console.warn(
          "Templates API returned invalid response, using empty list"
        );
        setTemplates([]);
        return;
      }
      ...
      const usedTemplateIds = new Set(
        existingBulkSends.map((bs) => bs.templateId)
      );

      // Filter out templates that are already used in bulk sends
      const availableTemplates = response.results.filter(
  ...
  const removeMemberFromSigners = (memberId: string) => {
    ...
    if (member) {
      ...
      setSelectedMembers(
        new Set([...selectedMembers].filter((id) => id !== memberId))
      );
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const loadingToastId = toast.loading(
        "Creating documents using OpenSign's batchdocuments function..."
      );

      try {
        const newBulkSend = await bulkSendApiService.createBulkSend(
          bulkSendData
        );
        ...
        toast.success(
          `🎉 Bulk send "${bulkSendName}" created successfully! ${signers.length} documents created.`
        );
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Create multiple documents from template • {signers.length}{" "}
                    signers added
                  </p>
                </div>
              </div>
                    ...
                    <div>
                      ...
                      {templates.length === 0 && !loadingTemplates && (
                        <p className="mt-1 text-xs text-yellow-600">
                          No available templates found. All templates may
                          already be used in existing bulk sends.
                        </p>
                      )}
                    </div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">
                          {selectedTemplate.description ||
                            "Template selected for bulk sending"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 text-sm leading-relaxed text-gray-700">
                      <div className="p-4 border border-green-200 rounded-lg bg-green-50">
                        <h4 className="mb-2 font-medium text-green-700">
                          How Bulk Send Works with OpenSign
                        </h4>
                        <ul className="space-y-2 text-green-600">
                          ...
                          <li className="flex items-start space-x-2">
                            <div className="flex-shrink-0 w-2 h-2 mt-2 bg-green-500 rounded-full"></div>
                            <span>
                              No custom backend classes needed - leverages
              ...
              <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
                ...
                <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-medium text-gray-900">
                      Add Signers from Team
                    </h3>
                    <span className="text-sm text-gray-500">
                      {signers.length} selected
                    </span>
                  </div>
                ...
                <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                  ...
                  {signers.length === 0 ? (
                    <div className="py-8 text-center">
                      ...
                      <p className="text-xs text-gray-400">
                        Select contacts from the left to add signers
                      </p>
                    </div>
                  ) : (
                ...
                <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {signers.length} documents will be created
                      </p>
                    </div>
