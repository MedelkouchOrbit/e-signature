...
export interface SuperAdminOrganizationWithUsers
  extends SuperAdminOrganization {
...
export class SuperAdminApiService {
  ...
  > {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          "Failed to fetch organizations via comprehensive discovery:",
          response.status,
          errorText
        );
      ...
      const organizations: SuperAdminOrganizationWithUsers[] = data.results.map(
        (org: unknown) => {
      ...
      console.log(
        `Found ${organizations.length} organizations with comprehensive discovery`
      );
      return organizations;
    } catch (error) {
      console.error(
        "Error getting organizations via comprehensive discovery:",
        error
      );
      throw error;
    }
  ...
  static async createOrganization(
    ...
    try {
      console.log(
        "� Creating organization via optimized addadmin transaction..."
      );

      const addAdminResponse = await fetch(
        "http://94.249.71.89:9000/api/app/functions/addadmin",
        {
          method: "POST",
          headers: {
      ...
      if (result.result?.message === "User already exist") {
        throw new Error(
          `Admin user with email "${data.adminUser.email}" already exists. Please use a different email address.`
        );
      }
      ...
      const foundOrg = await this.findOrganizationByName(
        data.name,
        sessionToken
      );
      ...
      console.log(
        "⚠️ Using synthetic response - organization created but not discoverable"
      );
      return {
      ...
      console.warn(
        "❌ AddAdmin method failed, attempting direct creation:",
        addAdminError
      );
      ...
      try {
        ...
        if (!orgResponse.objectId) {
          throw new Error(
            "Organization creation failed - no objectId returned"
          );
        }
        ...
        try {
          ...
          console.error(
            "❌ Admin user creation failed - transaction partially completed:",
            userError
          );
        ...
        console.error(
          "❌ Direct creation transaction failed:",
          directCreationError
        );
        const errorMessage =
          directCreationError instanceof Error
            ? directCreationError.message
        ...
        response.result?.map((user) => ({
          ...
          UserId: user.UserId
            ? {
                __type: "Pointer",
                className: "_User",
                objectId: "", // UserId.objectId is not provided in this response
              }
            : undefined,
          createdAt: "", // createdAt is not provided in this response
        })) || []
      );
    } catch (error) {
  ...
  static async createUser(data: CreateUserRequest): Promise<SuperAdminUser> {
    try {
      ...
      const userResponse = await openSignApiService.post<{
        ...
        Name: data.name,
        Email: data.email,
        UserRole: data.role.startsWith("contracts_")
          ? data.role
          : `contracts_${data.role}`,
        IsDisabled: false,
    ...
    try {
      ...
      if (directResponse.error) {
        ...
        if (fallbackResponse.error) {
          console.error(
            "Fallback teams query also failed:",
        ...
        console.log(
          `📋 Found ${activeTeams.length} active teams via fallback query (${allTeams.length} total)`
        );
        return activeTeams;
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          result.error?.includes("create on class contracts_Teams")
        ) {
...
Current Status: User is assigned to team ${
            result.currentTeam || "N6rqaITcpM"
          } but cannot create additional teams.`);
        }

        // Handle the "not implemented" case with a more user-friendly message
        if (response.status === 501) {
          throw new Error(
            `Team creation is not currently available. ${
              result.suggestion ||
              "Teams are automatically created when adding users to the organization."
            }`
          );
        }

        throw new Error(
          result.message || result.error || "Failed to create team"
        );
      }
  ...
  static async toggleTeamStatus(
    ...
    try {
      ...
      console.log(
        `Team ${teamId} status toggled to ${isActive ? "active" : "inactive"}`
      );
    } catch (error) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errorData = await response
          ...
          .catch(() => ({ error: "Unknown error" }));
        throw new Error(
          errorData.error || `Failed to get user details: ${response.status}`
        );
      }
  ...
  static async grantTeamAccess(userEmail: string): Promise<void> {
    try {
      ...
      if (!userDetails || !userDetails.OrganizationId) {
        console.warn(
          "⚠️ User details not found or no organization assigned, skipping team access grant"
        );
        return;
      }
      ...
      if (userDetails.TeamIds && userDetails.TeamIds.length > 0) {
        ...
        console.log(
          "� Admin user has team membership but may lack CREATE permissions"
        );
        console.log(
          "💡 Solution: Use the createTeam API endpoint which should handle permissions correctly"
        );
      ...
      console.warn(
        "⚠️ No team access found - this should not happen after addadmin"
      );
      ...
      console.log(
        "ℹ️ Team access verification failed, but user should still have basic access"
      );
    }
  }
}
